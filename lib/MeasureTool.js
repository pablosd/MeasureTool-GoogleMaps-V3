...
var ContextMenu = function () {
  ...
  _createClass(ContextMenu, [{
    ...
    value: function show(point) {
	  ...
	  this._containerDiv.style.cssText += 'visibility: visible; height: 52px;'
    }
  }, {
    key: 'hide',
    value: function hide() {
  ...
  _createClass(MeasureTool, [{
    ...
    value: function _init() {
      ...
      if (this._options.contextMenu) {
        this._contextMenu = new _contextMenu2.default(this._containerDiv, { width: 'auto' });
        this._startElementNode = this._contextMenu.addItem("Measure distance", false, this.start, this);
        this._endElementNode = this._contextMenu.addItem("Clear measurement", true, this.end, this);
    ...
    value: function start(initialPoints) {
      ...
      _this2._contextMenu.show()
      this._mapClickEvent = this._map.addListener('click', function (mouseEvent) {
	  if (_this2._mapLastClickEvent) {
		_this2._firstClick = mouseEvent
	  }
	  _this2._mapLastClickEvent = mouseEvent
    ...
    value: function _updateArea(i, pointToCompare) {
      ...
      this._area = area;
      if (area > 0) {
		this._nodeText.select(':last-child').text(this.lengthText);
    ...
    value: function _getResults() {
	  ...
	  let angle = 0
	  /// ////////////
	  let radians = function (n) {
		return n * (Math.PI / 180)
	  }
	  let degrees = function (n) {
		return n * (180 / Math.PI)
	  }

	  let getBearing = function (startLat, startLong, endLat, endLong) {
		...
		let angleResult = Math.round(Math.abs(
		  (degrees(Math.atan2(dLong, dPhi)) + 360.0) % 360.0))
		if (angleResult) {
		  if (angleResult < 180) {
	  ...
	  let destinationPoint = function (lat, lng, distance, bearing) {
		let radius = 6371e3
		const dr = distance / radius
		const br = radians(Number(bearing))
		...
		const y = Math.sin(br) * Math.sin(dr) * Math.cos(φ1)
		const x = Math.cos(dr) - Math.sin(φ1) * sinφ2
		const λ2 = λ1 + Math.atan2(y, x)

		const latResult = degrees(φ2)
		const lngResult = degrees(λ2)

		return {lat: latResult, lng: lngResult}
	  }
	/// ///////////
	  for (let i = 0; i < this.points.length; i++) {
	    arrayToCalculate.push([this.points[i].lng, this.points[i].lat])
	    let annotationPoint = this.points[i]
	    if (i > 0) {
	  	  if (this.points[i].lat >= this.points[i - 1].lat) {
	  	    annotationPoint =
		      destinationPoint(this.points[i].lat, this.points[i].lng,
			    3, 0)
		  } else {
		    annotationPoint =
		      destinationPoint(this.points[i].lat, this.points[i].lng,
	    ...
	    annotationsPoints.push({
	  	  text: this._helper.formatLength(
	        this._helper.computePathLength(arrayToCalculate)),
		  point: annotationPoint,
		  type: this.points.length - 1 === i ? 'TOTAL' : 'POINT',
		  angle: angle
	    })
	  }
	  let annotationsSegments = []
	  for (let i = 0; i < this.segments.length; i++) {
	    let p2 = [
	      this.segments[i].start_location.lng,
		  this.segments[i].start_location.lat
	    ]
	    let p1 = [
	      this.segments[i].end_location.lng,
		  this.segments[i].end_location.lat
	    ]
	    let point = [p2, p1]

	    angle = getBearing(p1[1], p1[0], p2[1], p2[0])

	    let transformedText = [
	      (point[0][0] + point[1][0]) / 2,
		  (point[0][1] + point[1][1]) / 2
	    ]
	    annotationsSegments.push({
	      text: this.segments[i].length.text,
		  point: destinationPoint(transformedText[1], transformedText[0],
		    2, angle),
		  type: 'SEGMENT',
		  angle: angle
	    })
	  ...
	  return {
	    result: {
		  ...
		  points: this.points,
		  annotationsPoints: annotationsPoints,
		  annotationsSegments: annotationsSegments
	    }
	  };
    }
  }], [{
